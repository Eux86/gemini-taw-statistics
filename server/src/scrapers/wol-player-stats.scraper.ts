...
export class WolPlayerStatsScraper implements IScraper {
  id = 'wol-player-stats';
  ...
  run = async (squadronName: string): Promise<undefined> => {
    ...
    do {
      try {
        console.log('Collecting sorties from Wol');
    ...
    console.log('Wol sorties collected');
    return;
  }

  private collect = async (squadronName: string): Promise<ISortie[]> => {
    const data = await getPageContent(`http://il2stat.aviaskins.com:8008/en/pilots/?search=${squadronName}`, true);
    ...
    for (let i = 0; i < playerPages.length; i++) {                                                                     // playerPages.length
      ...
      const playersSorties = await this.getPlayersSorties(`http://il2stat.aviaskins.com:8008${sortiesPageUrl}`);
  ...
  private getPlayersSorties = async (url: string): Promise<ISortie[]> => {
    ...
    for (let i = 0; i < playerSortiesUrls.length; i++) {
      ...
      const sortieDetails = await this.getSortieDetails(`http://il2stat.aviaskins.com:8008${sortieUrl}`);
  ...
  private getSortieDetails = async (url: string): Promise<ISortie> => {
    ...
    const hash = createHash(`${sortieDate}${playerName}wol`);
...
// new WolPlayerStatsScraper({} as Db).run('=GEMINI=').then(() => console.log('== done =='));
