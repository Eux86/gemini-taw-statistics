...
export class ItawPlayerStatsScraper implements IScraper {
  id = 'itaw-player-stats';
  ...
  run = async (squadronName: string): Promise<undefined> => {
    ...
    do {
      try {
        console.log('Collecting sorties from Itaw');
    ...
    console.log('Itaw sorties collected');
    return;
  }

  private collect = async (squadronName: string): Promise<ISortie[]> => {
    const data = await getPageContent(`http://88.99.165.102:8000/en/pilots/?search=${squadronName}`, true);
    ...
    for (let i = 0; i < playerPages.length; i++) {                                                                     // playerPages.length
      ...
      const playersSorties = await this.getPlayersSorties(`http://88.99.165.102:8000${sortiesPageUrl}`);
  ...
  private getPlayersSorties = async (url: string): Promise<ISortie[]> => {
    ...
    for (let i = 0; i < playerSortiesUrls.length; i++) {
      ...
      const sortieDetails = await this.getSortieDetails(`http://88.99.165.102:8000${sortieUrl}`);
  ...
  private getSortieDetails = async (url: string): Promise<ISortie> => {
    ...
    const hash = createHash(`${sortieDate}${playerName}itaw`);
...
// new ItawPlayerStatsScraper({} as Db).run('=GEMINI=').then(() => console.log('== done =='));
