...
export class FinnPlayerStatsScraper implements IScraper {
  id = 'finn-player-stats';
  ...
  run = async (squadronName: string): Promise<undefined> => {
    ...
    do {
      try {
        console.log('Collecting sorties from Finn');
    ...
    console.log('Finn sorties collected');
    return;
  }

  private collect = async (squadronName: string): Promise<ISortie[]> => {
    const data = await getPageContent(`http://ts3.virtualpilots.fi:8000/en/pilots/?search=${squadronName}`, true);
    ...
    for (let i = 0; i < playerPages.length; i++) {                                                                     // playerPages.length
      ...
      const playersSorties = await this.getPlayersSorties(`http://ts3.virtualpilots.fi:8000${sortiesPageUrl}`);
  ...
  private getPlayersSorties = async (url: string): Promise<ISortie[]> => {
    ...
    for (let i = 0; i < playerSortiesUrls.length; i++) {
      ...
      const sortieDetails = await this.getSortieDetails(`http://ts3.virtualpilots.fi:8000${sortieUrl}`);
  ...
  private getSortieDetails = async (url: string): Promise<ISortie> => {
    ...
    const hash = createHash(`${sortieDate}${playerName}finn`);
...
// new FinnPlayerStatsScraper({} as Db).run('=GEMINI=').then(() => console.log('== done =='));
